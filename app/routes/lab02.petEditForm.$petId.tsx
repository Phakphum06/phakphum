...
export default function PetEditForm(){
    ...
    useEffect(() => {
        try {
            const fetchPetData = async () => {
                const petData = await fetch(`http://localhost:5173/api/getPetById/${petId}`);
                if (petData.ok) {
                    const petJson = await petData.json();
                    setPetData(petJson);
                    console.log(petJson.petType);
                    console.log(petJson);
                } else {
    ...
    const handleSubmit = async (e) => {
      ...
      if(confirm('Confirm the information update?')){
        ...
        try {
            const resPet = await fetch('http://localhost:5173/api/updetePet', {
                method: 'POST',
                headers: {
